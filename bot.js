...
module.exports = class extends Command {
    ...
    async run(msg, [username]) {
        ...
        return msg.sendMessage(new MessageEmbed()
            ...
            .addField("❯ Total Kills", stats ? stats.find(a => a.name === "total_kills").value.toLocaleString() : 0, true)
            .addField("❯ Total Deaths", stats ? stats.find(a => a.name === "total_deaths").value.toLocaleString() : 0, true)
            .addField("❯ Total Wins", stats ? stats.find(a => a.name === "total_wins").value.toLocaleString() : 0, true)
            .addField("❯ Total MVPs", stats ? stats.find(a => a.name === "total_mvps").value.toLocaleString() : 0, true)
            .addField("❯ Time Played (Not Idle)", `${stats ? (stats.find(a => a.name === "total_time_played").value / 60 / 60).toFixed(2) : 0} Hour(s)`, true)
            .addField("❯ Knife Kills", stats ? stats.find(a => a.name === "total_kills_knife").value.toLocaleString() : 0, true));
    }
